...
void train(string filename,vector<map<int,double> > &olddelta)
{
	...
	if(filename.find(open)!=string::npos)
	{
		outputs[0]=0.1;
	}	
	else if(filename.find(sunglasses)!=string::npos)
	{
		outputs[0]=0.9;
	}
	else
...
double findaccuracy(char *testfile)
{
	...
	while(!g.eof())	
	{
		...
		//determining if person is wearing sunglasses or not
		int eye = -1;
		string open = "open";
		string sunglasses = "sunglasses";
		if(filename.find(open)!=string::npos)
...
int main(int argc, char **argv)
{
	...
	addDenseLayer(50 + 1,0); //Adding second layer with 6 hidden units plus a bias unit
	addDenseLayer(1,1); //Adding final layer with only one neuron
